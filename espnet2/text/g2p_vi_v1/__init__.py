...
from espnet2.text.g2p_vi_v1.symbols import symbols
...
class G2p_vi_v1:
    ...
    def build_phoneme(self, graph: str, _remove_vowel: bool=False) -> List[str]:
        ...
        if len(self.symbols) == 131:
            ...
            _phn = [_consonant, _medial, f"{_vowel}{tone}", _coda]
        else:
    ...
    def g2p(self, text: str) -> List:
        ...
        for i, word in enumerate(sequences):
            if self.en_words is not None and word in self.en_words:
                # if word in self.vn_words:
                #     print(word)
                word = self.en_words[word]["vietlish"]
            ...
            if "-" in word:
                ...
                # if any(x.endswith("ờ") for x in word): 
                #     print(f"[*] word(condition 1): {'-'.join(word)} => sequences: {sequences[i]}")
            else:
